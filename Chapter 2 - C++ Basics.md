...
두번째 시그니처를 좀 더 자세히 살펴보겠습니다. 두번째 시그니처는 2개의 매개 변수(```argc```, ```argv```)를 갖습니다. 매개 변수 ```argc```는 인수의 개수를 나타내고, ```argv```는 인수의 목록을 포함합니다. 특히, ```argv```의 첫번째 인자(```argv[0]```, 배열에 대한 내용은 2.8.1절 참조)에는 실행한 파일의 이름이 저장되어 있습니다. ```ArgcArgvTest```라는 프로그램을 통해 ```argc```와 ```argv```를 어떻게 사용하는지 알아봅시다.